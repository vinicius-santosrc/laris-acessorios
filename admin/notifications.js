document.querySelector('.notifications-blocks').innerHTML = `
                <div class="not-block">
                    <h2>Atualização 1 (07:23 de 22/07/2023)</h2>
                    <p>* Redefinição das cores".</p>
                    <p>* Aparência Escura e Clara disponíveis nos ajustes</p>
                    <p></p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 5 horas e 14 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.9 (02:09 de 22/07/2023)</h2>
                    <p>* Mudanças na "Planilha de Itens:".</p>
                    <p>--> Cores em determinados itens.</p>
                    <p>--> Centralização do texto em determinados itens.</p>
                    <p>--> Adição da nova opção "Custos"</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.8 (13:14 de 17/07/2023)</h2>
                    <p>* Sistema de mudança de plano de fundo na aba de Plano Semanal.</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.7 (10:27 de 29/06/2023)</h2>
                    <p>* Novo sistema de importação e exportação de dados.</p>
                    <p>* Mudança no estoque das pratas</p>
                    <p>* Correção de bugs</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.6 (18:57 de 24/06/2023)</h2>
                    <p>* Adição de produtos no estoque das pratas</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.5 (17:42 de 24/06/2023)</h2>
                    <p>* Finalização da página "Plano Mensal", disponível na página da administração.</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.4 (21:45 de 23/06/2023)</h2>
                    <p>Listagem oficial:</p>
                    <p>* Alteração no site da administração</p>
                    <p>* Mudança no estoque das pratas</p>
                    <p>* Início da estrutura básica do site dos pedidos pendentes.</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.3 (17:58 de 03/06/2023)</h2>
                    <p>Listagem oficial:</p>
                    <p>* Modelo para celular concluído</p>
                    <p>* Novas opções no menu que estarão disponíveis em breve</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.2 (20:27 de 30/05/2023)</h2>
                    <p>Listagem oficial:</p>
                    <p>* Opção "Overview" sendo desenvolvida. Opções que são esperadas:</p>
                    <p>* -> Visão Geral do Sistema LARI'S</p>
                    <p>* -> Média de Ganhos e Perdas Mensais</p>
                    <p>* -> Widgets Interativos</p>
                    <p>* Opção "Planilha", disponível no website da administração, foi lançada com êxito</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
                <div class="not-block">
                    <h2>Atualização 0.1 (23:00 de 17/05/2023)</h2>
                    <p>Mudanças no site oficial/admin</p>
                    <p>Listagem oficial:</p>
                    <p>* Mudanças no estoque das pratas</p>
                    <p>* Mudanças na estrutura da página da administração</p>
                    <p>* Mudanças no estilo da página da administração</p>
                    <p>Autor: Vinícius S.</p>
                    <p style="color: orange">Tempo gasto para atualização: 0 hora 0 minutos</p>
                    <p style="color: green">Uploaded: Sucess</p>
                </div>
`